...
class Assign extends MX_Controller {
	...
	{   
		...
		$data['title']    = display('reports');  
        #-------------------------------#
        $config["base_url"]   = base_url('reports/assign/report_vehicle');
        $config['suffix'] = '?'.http_build_query($_GET, '', "&"); 
        $config['first_url'] = $config['base_url'].$config['suffix'];
        $config["per_page"] = 10;
        $config["uri_segment"] = 4;
		...
		$data['module'] = "reports";
		$data['page']   = "assign/vehicle";   
		echo Modules::run('template/layout', $data); 
	}

    public function report_vehicle_trip()
	{   
        $this->permission->method('reports','read')->redirect();
		$data['title']    = display('reports');  
        #-------------------------------#
        $config["base_url"]   = base_url('reports/assign/report_vehicle_trip');
        $config['suffix'] = '?'.http_build_query($_GET, '', "&"); 
        $config['first_url'] = $config['base_url'].$config['suffix'];
        $config["per_page"] = 10;
        $config["uri_segment"] = 4;
