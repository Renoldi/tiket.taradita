       ...
       $price_info = $this->db->select('*')->from('pri_price')->where('route_id',$route_id)->where('vehicle_type_id',$type)->get();
        ...
        if ($this->form_validation->run()) {
            ...
            $postData = [
                ...
                'children_price'  => 0,
                'special_price'   => 0,
                'group_price_per_person' => $this->input->post('group_price_per_person'),
      ...
      $route_id = $this->input->post('route_id');
       ...
       $price_info = $this->db->select('*')->from('pri_price')->where('route_id',$route_id)->where('vehicle_type_id',$type)->get();
        ...
        if($price_info->num_rows() < 2){
            ...
            $data['module']    = "trip";    
            $data['page']      = "price/update_price_form";   
            echo Modules::run('template/layout', $data);  
        }   
    }
