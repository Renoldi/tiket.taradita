...
class Route extends MX_Controller {
	...
	{ 
     ...
     $output = implode(',', $array3);
			...
			$stopagp = (!empty($this->input->post('stoppage_points'))?$start->name.','.$output.','.$end->name:'');
		...
		$data['route'] = (Object) $postData = [
			...
			'pickup_village' 	 => $this->input->post('pickup_village'),
			'pickup_district' 	 => $this->input->post('pickup_district'),
			'drop_village' 	 => $this->input->post('drop_village'),
			'drop_district' 	 => $this->input->post('drop_district'),
