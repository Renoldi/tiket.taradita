...
class Search extends MX_Controller {
	...
	{   
		...
		$getFleetCategory  = $this->input->get('fleet_category');
		#---------------POST DATA------------------
		$postStartPoint = $this->input->post('start_point');
		$postEndPoint   = $this->input->post('end_point');
		$postFleetCategory  = $this->input->post('fleet_category');
		#---------------FINAL DATA------------------
		$data['search'] = (object) $postData = array(
			...
			'fleet_category'  => (!empty($postFleetCategory)?$postFleetCategory:$getFleetCategory),
		);
    ...
    */
    ...
    public function booking()
    {
        ...
        $this->form_validation->set_rules('adult', display('adult') , 'less_than[4]');
        ...
        $postData = array(
            ...
            'pickup_trip_village' => $this->input->post('pickup_village'),
            'pickup_trip_district' => $this->input->post('pickup_district'),
            'drop_trip_location'   => $this->input->post('drop_location'),
            'drop_trip_village' => $this->input->post('drop_village'),
            'drop_trip_district' => $this->input->post('drop_district'),
            ...
            'status'               => '0',
            'company_id' => $rout_chsp_seat->company_id
        );
        #--------------------------------------
 ...
 $cs = $this->db->select("
    ...
    */
    ...
    { 
        ...
        $this->form_validation->set_rules('phone', display('phone') ,'required|max_length[14]');
        $this->form_validation->set_rules('email', display('email') ,'valid_email|max_length[100]');
        $this->form_validation->set_rules('password', display('password') ,'min_length[5]');
        $this->form_validation->set_rules('address_line_1', display('address') ,'max_length[500]');
     ...
     $this->db->where('id_no', $this->input->post('booking_id_no',true))
      ...
      'company_id'     => $this->input->post('company_id',true)
        ...
        $obj['booking'] =$binfo= $this->db->select("
                ...
                foreach ($this->input->post('firstname1') as $key => $val) {
                    $obj[] = array(
                        $obj['firstname1']       = $this->input->post('firstname1')[$key],
                        $obj['lastname1']        = $this->input->post('lastname1')[$key],
                        $obj['nid1']             = $nid1[$key]
                    );
                }
        ...
        if ($this->form_validation->run())
        {
            if($this->website_model->email_check($this->input->post('email'))){
                $this->website_model->createUser($userData);     
    ...
    */  
    ...
    public function findBookingInformation()
    {
        ...
        $data['seats'] = "<h4 class=\"text-primary text-center\">Pilih Kursi Anda</h4>";
...
foreach ($seatArray as $seat) 
    ...
    foreach ($seatArray as $seat) 
        {
            ...
            {
                //adding a cental row
                if ((sizeof($seatArray)&0) == 1 && ($lastSeats == 0 || $lastSeats < $totalSeats)) {
                    continue;
                } else {
                    $data['seats'] .= "<div class=\"col-xs-2\">&nbsp;</div>";
                }
 ...
 }else if($seatlayout == 'hiace-10'){
    $data['seats'] .= "<div class=\"row\">";
    $data['seats'] .= "<div class=\"col-xs-2\">
        <div class='".(in_array($fleetSeats->seat_numbers, $bookedSeats->booked_serial)?("seat ladies"):("seat occupied ChooseSeat"))."' data-item=\"\">
        <div class=\"seat-body\">
            A1
            <span class=\"seat-handle-left\"></span>
            <span class=\"seat-handle-right\"></span>
    ...
    foreach ($seatArray as $seat) 
        ...
        if (sizeof($facilitiesArray) > 0) 
        {
            ...
            {
                if ($fa != "")
                $data['facilities'] .= "<input id=\"f$key\" name=\"request_facilities[]\" class=\"inline-checkbox\" type=\"checkbox\" value=\"$fa\" checked> <label for=\"f$key\">$fa</label> ";
            }
        }

    	#--------- VILLAGE Pickup -----------#
        $tripVillage = $this->db->select('pickup_village')
            ->from('trip_route')
        ...
        $villageArray = array();
        $villageArray = array_map('trim', explode(', ', $tripVillage->pickup_village));
        $data['village'] = "";
        foreach ($villageArray as $vx) 
        {
            $data['village'] .= "<option value='$vx'>$vx</option>";
        }

        #--------- DISTRICT Pickup -----------#
        $tripDistrict = $this->db->select('pickup_district')
            ->from('trip_route')
            ->where('id', $trip_route_id)
            ->get()
            ->row();
        ...
        $districtArray = array();
        $districtArray = array_map('trim', explode(',', $tripDistrict->pickup_district));
        $data['district'] = "";
        foreach ($districtArray as $dx) 
        {
            $data['district'] .= "<option value='$dx'>$dx</option>";
        }

        #--------- VILLAGE Drop -----------#
        $tripVillage1 = $this->db->select('drop_village')
            ->from('trip_route')
            ->where('id', $trip_route_id)
            ->get()
            ->row();
        ...
        $village1Array = array();
        $village1Array = array_map('trim', explode(',', $tripVillage1->drop_village));
        $data['village1'] = "";
        foreach ($village1Array as $vx1) 
        {
            $data['village1'] .= "<option value='$vx1'>$vx1</option>";
        }

        #--------- DISTRICT Drop -----------#
        $tripDistrict1 = $this->db->select('drop_district')
            ->from('trip_route')
            ->where('id', $trip_route_id)
            ->get()
            ->row();

        $district1Array = array();
        $district1Array = array_map('trim', explode(',', $tripDistrict1->drop_district));
        $data['district1'] = "";
        foreach ($district1Array as $dx1) 
        {
            $data['district1'] .= "<option value='$dx1'>$dx1</option>";
        }
